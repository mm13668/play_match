...
AI驱动的双人关系匹配测试微信小程序，基于字节跳动豆包AI大模型API和微信云开发。

## 项目介绍

「去玩匹配」是一个趣味关系测试小程序，用户输入两个人的姓名和关系类型，AI会生成专属的关系匹配分析报告，包含匹配分数、契合度分析、相处优势和关系建议。支持免费体验、广告解锁、微信支付、分享邀请等多种模式。

## 功能特性

- ✨ **AI智能分析** - 基于豆包大模型生成专业关系匹配报告
- 🎭 **多种关系类型** - 支持情侣、朋友、同事、家人四种关系
- 🔓 **多种解锁方式** - 免费次数、观看广告、微信支付解锁完整报告
- 📜 **历史记录** - 自动保存所有测试记录，随时查看
...
7. **配置域名白名单**
   ...
   ```
   （旧版使用 `https://aquasearch.bytedance.com`）
...
- 严格相等 `===`/`!==`
- 驼峰命名变量，大写下划线分隔常量
- 异步函数使用 `async/await` + `try/catch`

### 云函数开发规范
- 统一使用 `wx-server-sdk`
- 返回结构化响应 `{success: true, data: ...}` 或 `{success: false, error: ...}`
- 敏感配置存放于独立 `config.js`，该文件已加入 `.gitignore`
...
**Q: AI返回解析错误怎么办？**  
A: 项目已有容错机制，AI返回异常时会使用默认结果。检查API Key和网络配置。
